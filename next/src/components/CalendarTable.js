...
export default async function CalendarTable() {
  ...
  return (
    <div className="overflow-auto">
      <table className="w-full border-collapse text-left min-w-[1100px] border border-white">
        <thead>
        ...
        <tbody>
          {data.calendar.map((week, weekIndex) => {
            const numberOfRows = week.rows.length;
            return week.rows.map((row, rowIndex) => (
              <CalendarRow
                key={`${weekIndex}-${rowIndex}`}
                weekNumber={week.weekNumber}
                row={row}
                rowSpan={rowIndex === 0 ? numberOfRows : undefined}
              />
            ));
          })}
        </tbody>
